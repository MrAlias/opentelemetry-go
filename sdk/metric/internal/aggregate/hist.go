...
func newHistogram[N int64 | float64](cfg aggregation.ExplicitBucketHistogram, t metricdata.Temporality, f attribute.Filter) (Input[N], Output) {
	...
	if f == nil {
		...
		mapper := filterHistogramDataPoint(f, foldHist[N])
		out = func(dest *metricdata.Aggregation) {
			hData, _ := (*dest).(metricdata.Histogram[N])
			set(&hData)
			hData.DataPoints = mapper(hData.DataPoints)
			*dest = hData
		}
	}
	return in, out
